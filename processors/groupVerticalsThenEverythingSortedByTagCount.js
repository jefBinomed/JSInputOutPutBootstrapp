...
function getSlidesFromVerticals(verticals) {
    ...
    const sortedSlides = R.sort(R.descend(getSlideTagsCount), slides);

    return sortedSlides;
}

function getSlidesFromHorizontals(horizontals) {
    const getPhotoTagsCount = R.compose(R.length, R.pluck('tags'));

    const sortedHorizontals = R.sort(R.descend(getPhotoTagsCount), horizontals);

    return sortedHorizontals.map(photo => [photo]);
}
